      ...
      <nav className='relative z-20  text-black flex justify-between items-center py-4'>
          ...
          <div  className="hidden lg:flex items-center gap-8">
            {navLinks.map((item,key)=>{
              return(
                <Link key={key} to='/' className='text-base font-medium tracking-tight text-gray-500 hover:text-black'>{item.title}</Link>
              )
            })}
            <Link to="/" className='h-10 px-6 flex justify-center items-center text-white font-medium tracking-tight bg-black hover:bg-black/80  rounded-full'>Contact Me</Link>
          </div>
          {/* Menu Mobile */}
          <div className={` lg:hidden absolute top-14 w-full h-auto py-5 px-3 bg-black rounded-xl ${!expendMenu && 'opacity-0'}`}>
          {navLinks.map((item,key)=>{
              return(
                <Link key={key} to='/' className='block p-3 py-4 text-lg font-medium text-gray-400'>{item.title}</Link>
              )
            })}
